		...
		<div  style=" text-align: center ;">
			<nav aria-label="Page navigation" style="align-self: center;">
				<ul class="pagination">
					...
					<c:if test="${pageBean.pageNo != 1}">
						<li>
							<a href="${pageContext.request.contextPath}/student?method=pageList&pageNo=${pageBean.pageNo - 1}&pageSize=20"
								aria-label="Previous"> 
					...
					<c:forEach var="i" begin="1" end="${pageBean.totalPage}" step="1">
						...
						<c:if test="${pageBean.pageNo != i}">
							<li><a href="${pageContext.request.contextPath}/student?method=pageList&pageNo=${i}&pageSize=20">${i}</a></li>
						</c:if>
					</c:forEach>
					<!--中间页码结束  -->
					<!--下一页开始  -->

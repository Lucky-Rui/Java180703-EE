			...
			<div class="row">
				...
				<div class="col-md-10">
					<table class="table table-hover">
						<tr>
							<th><input type="checkbox" id="selectAlls" onclick="selectAll()"/>全选</th>
							<th>ID</th>
							<th>班级名称</th>
							<td>修改</td>
							<td>删除</td>
							...
							<tr>
								...
								<td>${banji.name}</td>
								<td>
									<a href="${pageContext.request.contextPath}/banji?method=toUpdate&id=${banji.id}">修改</a></td>
								<td>
		...
		<div  style=" text-align: center ;">
			<nav aria-label="Page navigation" style="align-self: center;">
				<ul class="pagination">
					...
					<c:if test="${pageBean.pageNo != 1}">
						<li>
							<a href="${pageContext.request.contextPath}/banji?method=pageList&pageNo=${pageBean.pageNo - 1}&pageSize=20"
								aria-label="Previous"> 
					...
					<c:forEach var="i" begin="1" end="${pageBean.totalPage}" step="1">
						...
						<c:if test="${pageBean.pageNo != i}">
							<li><a href="${pageContext.request.contextPath}/banji?method=pageList&pageNo=${i}&pageSize=20">${i}</a></li>
						</c:if>
					</c:forEach>
					<!--中间页码结束  -->
					<!--下一页开始  -->

<%@page import="com.situ.weekend07.banji.entity.BanJi_PageBean"%>
...
<body>
	<%
		BanJi_PageBean banJi_PageBean = (BanJi_PageBean) request.getAttribute("banJi_PageBean");
	%>
	...
	<form action="" id="mainForm" method="post">
		<table
			...
			<c:forEach items="${banJi_PageBean.list}" var="banji">
				<tr>
					...
					<td>
						<a href="javascript:deleteById(${banji.id},${banJi_PageBean.pageNo})">删除</a>
					</td>
				</tr>
			</c:forEach>
		</table>
	...
	<div  style=" text-align: center ;">
		<nav aria-label="Page navigation" style="align-self: center;">
			<ul class="pagination">
				<!--上一页开始  -->
				<c:if test="${banJi_PageBean.pageNo == 1}">
					<li class="disabled">
				...
				<c:if test="${banJi_PageBean.pageNo != 1}">
					<li>
						<a href="<%=request.getContextPath()%>/banji?method=pageList&pageNo=${banJi_PageBean.pageNo - 1}&pageSize=3"
							aria-label="Previous"> 
				...
				<!--上一页结束  -->
				<!--中间页码开始 -->
				<c:forEach var="i" begin="1" end="${banJi_PageBean.totalPage}" step="1">
					<c:if test="${banJi_PageBean.pageNo == i}">
				...
				<!--中间页码结束  -->
				<!--下一页开始  -->
				<c:if test="${banJi_PageBean.pageNo == banJi_PageBean.totalPage}">
					<li class="disabled">
